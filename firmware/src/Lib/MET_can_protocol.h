...
/// Defines the default Status Register's Index
enum{
    _STAT_REVISION_IDX = 0,
    _STAT_SYSTEM_IDX = 1,
    _STAT_ERRORS_IDX = 2,
    _STAT_COMMAND_IDX = 3,
    _STAT_AVAILABLE_IDX = 4,
};
...
typedef struct {
    ...
    // Default Status registers
    uint8_t revisionStatus[4];
    uint8_t systemStatus[4];
    uint8_t errorsStatus[4];
    uint8_t commandStatus[4];
    
    MET_CAN_PROTOCOL_READ_STATUS_CALLBACK readStatusHandler;
} MET_Protocol_Data_t;
