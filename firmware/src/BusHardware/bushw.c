...
void BusHwLoop(void){
    ...
    if(uc_CMP_ON_back != uc_CMP_ON_Get()){
        ...
        SETBIT_PROTOCOL_SYSTEM_COMPRESSION_ON(uc_CMP_ON_back);
    }
    
    if(uc_ROT_CW_back != uc_ROT_CW_Get()){
        uc_ROT_CW_back = uc_ROT_CW_Get();
    }else{
        SETBIT_PROTOCOL_SYSTEM_ROT_CW(uc_ROT_CW_back);
    }
    
    if(uc_ROT_CCW_back != uc_ROT_CCW_Get()){
        uc_ROT_CCW_back = uc_ROT_CCW_Get();
    }else{
        SETBIT_PROTOCOL_SYSTEM_ROT_CCW(uc_ROT_CCW_back);
    }
