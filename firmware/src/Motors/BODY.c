...
#define SPEED_DENOMINATOR       120
 ...
 const MET_canOpen_Config_t BODY_CONFIG_VECTOR[]={
...
MET_canOpen_Config_t BODY_POSITIONING[] = {
...
static int32_t bodyAnalogToPositionConversion(uint16_t analog){
    return (((int32_t) analog - ZERO_ENCODER) * 35156 / (ENCODER_ROUNDS));       
}

static int32_t bodyPositionToEncoderConversion(int32_t position){           
    return cGRAD_TO_ENCODER(position);    
}

static int32_t bodyEncoderToPositionConversion(int32_t encoder){           
    return ENCODER_TO_cGRAD(encoder);    
}

void bodyInitialization(canOpen_workflowData_t* pData){
    pData->deviceId = 0x5;    
    pData->config = BODY_CONFIG_VECTOR;
    pData->positionRegisters = BODY_POSITIONING;
    
    pData->getAnalogToPositionConversion  = bodyAnalogToPositionConversion;
    pData->getPositionToEncoderConversion = bodyPositionToEncoderConversion;
    pData->getEncoderToPositionConversion = bodyEncoderToPositionConversion;    
}
