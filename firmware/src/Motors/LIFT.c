...
#define SPEED_DENOMINATOR       120
 ...
 const MET_canOpen_Config_t LIFT_CONFIG_VECTOR[]={
...
MET_canOpen_Config_t LIFT_POSITIONING[] = {
...
static int32_t liftAnalogToPositionConversion(uint16_t analog){
    return (((int32_t) analog - ZERO_ENCODER) * 35156 / (ENCODER_ROUNDS));       
}

static int32_t liftPositionToEncoderConversion(int32_t position){           
    return cGRAD_TO_ENCODER(position);    
}

static int32_t liftEncoderToPositionConversion(int32_t encoder){           
    return ENCODER_TO_cGRAD(encoder);    
}

void liftInitialization(canOpen_workflowData_t* pData){
    pData->deviceId = 0x4;    
    pData->config = LIFT_CONFIG_VECTOR;
    pData->positionRegisters = LIFT_POSITIONING;
    
    pData->getAnalogToPositionConversion  = liftAnalogToPositionConversion;
    pData->getPositionToEncoderConversion = liftPositionToEncoderConversion;
    pData->getEncoderToPositionConversion = liftEncoderToPositionConversion;
